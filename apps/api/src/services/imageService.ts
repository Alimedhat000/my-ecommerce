...
export function extractPublicIdFromUrl(url: string): string | null {
  try {
    ...
    throw new Error(`Failed to extract id from image url: ${error.message}`);
  }
}
...
export async function deleteImage(id: number) {
  try {
    ...
    if (publicId) {
      ...
      } catch (_cloudinaryError: any) {
        console.warn('Failed to delete from Cloudinary:', _cloudinaryError.message);
